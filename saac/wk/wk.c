...
#ifdef _ADD_AC_IP				 // WON ADD ��AC_IP��ͨѶЭ����
			sprintf(buffer,"ɾ�����%s %s %s\n",idbuf,pasbuf, AC_IP);
#else
			sprintf(buffer,"ɾ�����%s %s\n",idbuf,pasbuf);
#endif
...
#ifdef _ADD_AC_IP				 // WON ADD ��AC_IP��ͨѶЭ����
			sprintf(buffer,"������Ϸ��%s %s %s\n",idbuf,pasbuf, AC_IP);
#else
			sprintf(buffer,"������Ϸ��%s %s\n",idbuf,pasbuf);
#endif
    ...
    while(1){
        ...
        case 'h':
            fprintf( stderr,
                     ...
                     "            [-a CS��ַ] [-c CS�˿�]\n"
                     "Ĭ�ϵ���ģ��Ϊ0.\n"
                     "\nCopyright 2003 ��zoro������ / Longzoro system supply\n"
                     );
            exit(0);
            break;
        // Spock 2000/12/5
...
int main( int argc , char ** argv )
{
    ...
    if( strlen(WGSAddress) < 8 || strlen(WGSAddress) > 16 ){
    	fprintf( stderr , "��ָ��CS��IP��ַ����ȷ.\n");
    	exit(1);
    }
    if( WGSPort < 0 ){
    	fprintf( stderr , "��ָ��CS�Ķ˿ڲ���ȷ.\n");
    	exit(1);
    }
#ifndef _FIX_WORKS    	
    fprintf( stderr , "\n�˿�:[%d] ����ģʽ:[%d] CS��ַ:[%s] CS�˿�:[%d]\n" ,
